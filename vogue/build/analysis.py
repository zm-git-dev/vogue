...
def build_analysis(multiqc_dict: dict, analysis_type: str):
    ...
    if analysis_type in m.ANALYSIS_SETS.keys():

        analysis = dict()

        # Find common analysis between predefined set and config file loaded.
        analysis_sets = set(m.ANALYSIS_SETS[analysis_type].keys()) & set(
        ...
        LOG.warning(f'Analysis did not match the analysis type: {analysis_type}')
        return None
