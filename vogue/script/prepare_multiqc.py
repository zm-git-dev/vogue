...
def prepare_multiqc(
    multiqc_json, log_level, output_json, decompose, sample, directory, regex_match
):
    """
    ...
    if decompose and not sample:
        LOG.error("Decompose mode needs list of samples. Choose one from above.")
        raise click.Abort()
    ...
    if validate_multiqc(multiqc_dict):
        ...
        multiqc_dict = extract_analysis(multiqc_dict=multiqc_dict, json_keys="all")
    else:
    ...
    LOG.info("Found following samples in valid modules: %s", samples_in_multiqc)
    ...
    if sample:
        ...
        LOG.info("Found the following valid samples in valid modules: %s", valid_samples)
    ...
    if decompose:
        ...
        LOG.info("Only the following modules will be extracted: %s", analysis_common_keys)
        multiqc_dict = extract_analysis(
            multiqc_dict=multiqc_dict, json_keys=analysis_common_keys, samples=valid_samples
        )
