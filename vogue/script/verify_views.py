...
processes=lims.get_processes(
    ...
    udf={MASTER_STEPS_UDFS['concentration_and_nr_defrosts']['lot_nr_udf']:'20139912'})
conc_defrosts={}
...
for sample, date in samples_2017.items():
    artifacts = lims.get_artifacts(process_type=MASTER_STEPS_UDFS['concentration_and_nr_defrosts']['lot_nr_step'], 
                       samplelimsid=sample.id)
    if artifacts:
        art=artifacts[-1]
        lotnr =  art.parent_process.udf.get(MASTER_STEPS_UDFS['concentration_and_nr_defrosts']['lot_nr_udf'])
    ...
    if artifacts:
        ...
        for art in inarts:
            if art.samples[0].id != sample.id:
                continue
            conc =  art.udf.get(MASTER_STEPS_UDFS['final_conc_and_amount_dna']['concentration_udf'])
    artifacts = lims.get_artifacts(process_type=MASTER_STEPS_UDFS['final_conc_and_amount_dna']['amount_step'], 
                       samplelimsid=sample.id)
    if artifacts:
        inarts=artifacts[-1].parent_process.all_inputs()
        for art in inarts:
            if art.samples[0].id != sample.id:
                continue
            amount =  art.udf.get(MASTER_STEPS_UDFS['final_conc_and_amount_dna']['amount_udf'])
            if amount>200:
                amount=200
...
mic_tags_=app.db.get_collection('application_tag').find({'category':{'$eq':'mic'}})
mic_tags = []
...
for sample, date in samples_2018.items():
    ...
    #get output artifacts from the correct step that are related to the sample 
    artifacts = lims.get_artifacts(process_type=MASTER_STEPS_UDFS['microbial_library_concentration']['concentration_step'], 
                       samplelimsid=sample.id)
    #get the input artifacts that holds the concentration udf
    #append the condentration to the correct month
    if artifacts:
        ...
        for art in inarts:
            if art.samples[0].id != sample.id:
                continue
            conc =  art.udf.get(MASTER_STEPS_UDFS['microbial_library_concentration']['concentration_udf'])
            month = date.month
            if conc is None:
                continue
...
def get_prepared_date(sample)-> dt:
    ...
    """
    artifacts = lims.get_artifacts(samplelimsid = sample.id, process_type = MASTER_STEPS_UDFS['prepared']['steps'])
    if not artifacts:
        return None
