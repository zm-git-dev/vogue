...
def inspect_analysis_result(analysis_dict: dict):
    ...
    if case_analysis_type == "multiqc":
        ...
        LOG.warning("Analysis type %s is not supported for cleanup",
                    case_analysis_type)
        return None
    ...
    analysis_names = [
        e for key in analysis_model.ANALYSIS_SETS
    ...
    valid_analysis = [
        e for e in list(analysis_dict.keys()) if e in analysis_names
    ]
    ...
    LOG.info(
        f'The following keys were found in the input config: {valid_analysis}')
