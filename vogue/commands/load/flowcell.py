...
@click.option('-r',
              '--run-id',
              help='Run id for the run. Eg: 190510_A00689_0032_BHJLW2DSXX')
@click.option('-a',
              '--all-runs',
              is_flag=True,
              help='Loads all flowcells found in LIMS.')
@click.option('--dry',
              is_flag=True,
...
def flowcell(run_id, all_runs, dry, days):
    ...
    if days:
        ...
        for run_type in RUN_TYPES:
            runs.extend(
                lims.get_processes(udf={'Run ID': run_id}, type=run_type))
        if runs == []:
            LOG.warning("There is no run with this Run ID")
            raise click.Abort()
        if len(runs) > 1:
            LOG.warning(
                "There is more than one run with this run ID. Picking the latest"
            )
            run = runs[-1]
        else:
            run = runs[0]
