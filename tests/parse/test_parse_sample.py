...
from vogue.parse.build.sample import (
    get_number_of_days, get_output_artifact, get_latest_input_artifact,
    str_to_datetime, get_concentration_and_nr_defrosts,
...
def test_get_latest_output_artifact_no_art(lims):
    ...
    latest_output_artifact = get_output_artifact(process_type,
                                                 lims_id,
                                                 lims,
                                                 last=True)
...
def test_get_latest_output_artifact(lims, lims_sample):
    ...
    # WHEN running _get_latest_output_artifact
    latest_output_artifact = get_output_artifact(process_type_name,
                                                 lims_sample.id,
                                                 lims,
                                                 last=True)
...
def test_get_latest_input_artifact(lims):
    ...
    latest_input_artifact = get_latest_input_artifact(process_type_name,
                                                      sample_id, lims)
...
def test_get_concentration_and_nr_defrosts(lims, lims_sample):
    ...
    P1 = lims._add_process(date_str='1919-01-01',
                           process_type=lot_nr_step_type)
    P1.udf[lot_nr_udf] = lotnumber
    P2 = lims._add_process(date_str='1819-01-01',
                           process_type=lot_nr_step_type)
    P2.udf[lot_nr_udf] = lotnumber
    P3 = lims._add_process(date_str='1719-01-01',
                           process_type=lot_nr_step_type)
    ...
    P4 = lims._add_process(date_str='1818-01-01',
                           process_type=concentration_step_type)
    ...
    result_dict = get_concentration_and_nr_defrosts(application_tag,
                                                    lims_sample.id, lims)

    # THEN concentration should be 12 and nr_defrosts 3
    assert result_dict['concentration'] == 12 and result_dict[
        'nr_defrosts'] == 3
...
def test_get_final_conc_and_amount_dna(lims, lims_sample):
    ...
    amount_step_type = lims._add_process_type(
        name='CG002 - Aggregate QC (DNA)')
    P1 = lims._add_process(date_str='1919-01-01',
                           process_type=amount_step_type,
                           pid='P1')
    ...
    concentration_step_type = lims._add_process_type(
        name='CG002 - Aggregate QC (Library Validation)')
    P2 = lims._add_process(date_str='1919-01-01',
                           process_type=concentration_step_type,
                           pid='P2')
    ...
    return_dict = get_final_conc_and_amount_dna(application_tag,
                                                lims_sample.id, lims)
...
def test_get_microbial_library_concentration(lims):
    ...
    concentration = get_microbial_library_concentration(
        application_tag, sample_id, lims)
...
def test_get_library_size_pre(lims, lims_sample):
    ...
    process_type = lims._add_process_type(
        name='CG002 - Amplify Adapter-Ligated Library (SS XT)')
    date1, date2, date3 = '2018-01-01', '2018-02-01', '2018-03-01'
    ...
    size = get_library_size(application_tag, lims_sample.id, lims,
