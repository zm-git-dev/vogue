...
def test_genotype(database):
    ...
    added_sample = app.adapter.genotype_analysis_collection.find_one({'_id': sample_id})
    assert app.adapter.genotype_analysis_collection.estimated_document_count() == 1
...
def test_genotype_no_id(database):
    ...
    assert app.adapter.genotype_analysis_collection.estimated_document_count() == 0
